...
class MultipleChoiceQuestion extends Question {
  ...
  MultipleChoiceQuestion(
      String title, List<String> options, this.correctAnswers)
      : super(title, options);
  ...
  bool isCorrect(List<int> selectedOptions) {
    ...
    return ListEquality()
        .equals(correctAnswers..sort(), selectedOptions..sort());
  }
}
...
class Participant {
  ...
  void calculateScore(List<Question> questions) {
    for (int i = 0; i < questions.length; i++) {
      ...
      print(
          'Chosen answer(s): ${selectedOptions.map((idx) => idx + 1).join(", ")}');
    }
  }
}
...
void main() {
  ...
  quiz.addQuestion(
      SingleChoiceQuestion("What is Dart?", ["Language", "Game", "Bird"], 0));
